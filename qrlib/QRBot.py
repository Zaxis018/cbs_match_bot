    ...
    def setup_platform_components(self, *args, **kwargs):
        ...
        for vault_name in QREnv.VAULT_NAMES:
            try:
                ...
                run_item.logger.error(
                    f"Failed to retrieve vault: {vault_name}")
                run_item.notification.set(subject=f"Warning! {QREnv.BOT_NAME} bot has stopped unexpectedly", data={
                                          "reason": f"Failed to retrieve vault: {vault_name}"})
                raise e

        for queue_name in QREnv.QUEUE_NAMES:
            try:
                QRQueue(queue_name)
                run_item.logger.info(
                    f"Queue accessibility checked: {queue_name}")
            except Exception as e:
                run_item.logger.error(f"Queue not accessible: {queue_name}")
                run_item.notification.set(subject=f"Warning! {QREnv.BOT_NAME} bot has stopped unexpectedly", data={
                                          "reason": f"Queue not accessible: {queue_name}"})
                raise e
        ...
        for storage_name in QREnv.STORAGE_NAMES:
            try:
                ...
                run_item.logger.info(
                    f"Storage bucket accessibility checked: {storage_name}")
            except Exception as e:
                run_item.logger.error(
                    f"Storage bucket not accessible:  {storage_name}")
                run_item.notification.set(subject=f"Warning! {QREnv.BOT_NAME} bot has stopped unexpectedly", data={
                                          "reason": f"Storage bucket not accessible: {storage_name}"})
                raise e
